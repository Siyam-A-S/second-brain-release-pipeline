create table if not exists public.account_entitlement_settings (
  key text primary key,
  value integer not null check (value >= 0),
  updated_at timestamptz not null default now()
);

insert into public.account_entitlement_settings (key, value)
values
  ('free_daily_request_limit', 250),
  ('pro_daily_request_limit', 1000)
on conflict (key) do nothing;

alter table public.account_entitlement_settings enable row level security;

revoke all on public.account_entitlement_settings from anon;
revoke all on public.account_entitlement_settings from authenticated;
grant select, update on public.account_entitlement_settings to service_role;
...
create index if not exists account_usage_daily_user_updated_idx
on public.account_usage_daily (user_id, updated_at desc);

alter table public.account_usage_daily enable row level security;

drop policy if exists "Users can read their own daily usage" on public.account_usage_daily;

create policy "Users can read their own daily usage"
on public.account_usage_daily
for select
to authenticated
using (auth.uid() = user_id);

grant select on public.account_usage_daily to authenticated;
grant select, insert, update on public.account_usage_daily to service_role;

drop function if exists public.consume_proxy_usage(uuid, integer);
...
begin
  ...
  update public.account_usage_daily
  set
    plan_name = v_plan_name,
    request_limit = v_request_limit,
    reset_at = v_reset_at,
    used = coalesce(used, 0) + p_increment,
    updated_at = now()
  where user_id = p_user_id
    and bucket_date = v_bucket_date
    and coalesce(used, 0) + p_increment <= request_limit
  returning * into v_usage;
  ...
  select *
  into v_usage
  from public.account_usage_daily
  where user_id = p_user_id
    and bucket_date = v_bucket_date;
